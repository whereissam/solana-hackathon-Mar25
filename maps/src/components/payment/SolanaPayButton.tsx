...
const SolanaPayButton: React.FC<SolanaPayButtonProps> = ({
  ...
  return (
    <Box
      className={styles.solanaPayContainer}
      sx={{
        display: "flex",
        flexDirection: "column",
        gap: 2,
    ...
    >
      <Button
        ...
        className={`${styles.payButton} ${loadingWallet ? styles.processingButton : ''}`}
        fullWidth
        sx={{
      ...
      {walletError && !isProcessing && (
        <Alert severity="error" className={styles.alert} sx={{ width: "100%" }}>
          {walletError}
        </Alert>
      )}

      {!wallet.connected && !isProcessing && (
        <Alert severity="info" className={styles.alert} sx={{ width: "100%" }}>
